<template>
  <div :class="cls">
    ...
    <slot name="indicator">
      <div v-if="showIndicators && count > 1" :class="indicatorsCls">
        <i
          v-for="(empty, index) in Array(...Array(count))"
          :key="index"
          :style="index === activeIndicator ? indicatorStyle : null"
          :class="_getIndicatorCls(index)"
        />
      </div>
    </slot>
  </div>
...
export default createComponent({
  ...
  computed: {
    ...
    indicatorsCls() {
      return bem('indicators', { vertical: this.vertical });
    },

    indicatorCls(index) {
      return bem('indicator', { active: index === this.activeIndicator });
    },
