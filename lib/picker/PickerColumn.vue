  ...
  >
    ...
    >
      <li
        ...
        v-html="allowHtml ? getOptionText(option) : false"
        :key="index"
        :style="{height: `${itemHeight}px`}"
        :class="_getItemCls(option, index)"
        @click="onClickItem(index)"
      >{{ allowHtml ? '' : getOptionText(option) }}</li>
    </ul>
  </div>
</template>
...
export default createComponent({
  ...
  computed: {
    ...
    wrapperStyle() {
      const {
        offset, duration, baseOffset, itemHeight
      } = this;
      return {
